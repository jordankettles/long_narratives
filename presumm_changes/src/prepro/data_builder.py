...
class BertData():
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        if ((not is_test) and len(src) < self.args.min_src_nsents):
            print("src had only " + str(len(src)) + " sentences")
            return None
...
def _format_to_bert(params):
    ...
    datasets = []
    source, sent_labels = jobs['src'], jobs['tgt']
    tgt = ''
    # rouge would have been called here
    ...
    b_data_dict = {"src": src_subtoken_idxs, "tgt": tgt_subtoken_idxs,
                    ...
                    'src_txt': src_txt, "tgt_txt": tgt_txt, "filename": save_file.split('\\')[-1].replace(".valid.bert.pt", "")}
