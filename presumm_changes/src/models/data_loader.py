...
def load_dataset(args, corpus_type, shuffle):
    ...
    pts = sorted(glob.glob(args.bert_data_path + '*' + corpus_type + '.*.pt')) # Normal one.
    # pts = sorted(glob.glob(args.bert_data_path + '*' + '.*.pt')) # Training Point all
    if pts:
        if (shuffle):
            random.shuffle(pts)
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        if(is_test):
            ...
            return src, tgt, segs, clss, src_sent_labels, src_txt, filename
        else:
